		...
		private int checkLongOption()
		{
			...
			if (flag && !flag2)
			{
				if (this.opterr)
				{
                    ...
                    Console.WriteLine("{0}: option ''{1}'' is ambiguous", arg);
				}
				this.nextchar = "";
				this.optopt = 0;
				this.optind++;
				...
				if (longOpt.HasArg == Argument.No)
				{
					if (this.opterr)
					{
						if (this.argv[this.optind - 1].StartsWith("--"))
						{
                            ...
                            Console.WriteLine("{0}: option ''--{1}'' doesn't allow an argument", arg2);
						}
						else
						{
                            ...
                            Console.WriteLine("{0}: option ''{1}{2}'' doesn't allow an argument", arg3);
						}
					}
					this.nextchar = "";
				...
				if (longOpt.HasArg == Argument.Required)
				{
					...
					else
					{
						if (this.opterr)
						{
                            ...
                            Console.WriteLine("{0}: option ''{1}'' requires an argument", arg4);
						}
						this.nextchar = "";
						this.optopt = longOpt.Val;
						if (this.optstring[0] == ':')
				...
				if (!this.longOnly || this.argv[this.optind].StartsWith("--") || this.optstring.IndexOf(this.nextchar[0]) == -1)
				{
					if (this.opterr)
					{
						if (this.argv[this.optind].StartsWith("--"))
						{
                            ...
                            Console.WriteLine("{0}: unrecognized option ''--{1}''", arg);
						}
						else
						{
							object[] arg2 = new object[]
                            ...
                            Console.WriteLine("{0}: unrecognized option ''{1}{2}''", arg2);
						}
					}
					this.nextchar = "";
					this.optind++;
			...
			if (text == null || num == 58)
			{
				if (this.opterr)
				{
					if (this.posixlyCorrect)
					{
                        ...
                        Console.WriteLine("{0}: illegal option -- {1}", arg3);
					}
					else
					{
						object[] arg4 = new object[]
                        ...
                        Console.WriteLine("{0}: invalid option -- {1}", arg4);
					}
				}
				this.optopt = num;
				return 63;
						...
						else
						{
							if (this.optind == this.argv.Length)
							{
								if (this.opterr)
								{
                                    ...
                                    Console.WriteLine("{0}: option requires an argument -- {1}", arg5);
								}
								this.optopt = num;
								if (this.optstring[0] == ':')
								{
								...
								if (this.posixlyCorrect && this.optarg.Equals("--"))
								{
									if (this.optind == this.argv.Length)
									{
										if (this.opterr)
										{
                                            ...
                                            Console.WriteLine("{0}: option requires an argument -- {1}", arg6);
										}
										this.optopt = num;
										if (this.optstring[0] == ':')
										{
			...
			else
			{
				if (this.optind == this.argv.Length)
				{
					if (this.opterr)
					{
                        ...
                        Console.WriteLine("{0}: option requires an argument -- {1}", arg7);
					}
					this.optopt = num;
					if (this.optstring[0] == ':')
					{
